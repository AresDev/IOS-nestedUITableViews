...
- (NSInteger)numberOfSectionsInTableView:(UITableView *)tableView {
    ...
    return tableView.tag == 100 ? 1:1;
}

- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    // Return the number of rows in the section.
    return tableView.tag == 100 ? 3:4;
}


- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    
    NSString* cellIdentifier = tableView.tag == 100 ? @"parentCell":@"childCell";
    UITableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:cellIdentifier forIndexPath:indexPath];
    
    UITableView* childTableView;
    switch (tableView.tag) {
        case  100:
                childTableView = (UITableView*)[cell.contentView.subviews firstObject];
                if (childTableView) {
